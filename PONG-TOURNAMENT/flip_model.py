...
def flip_nature_cnn_weights(model_path, save_path):
    ...
    # Load the model to CPU to allow easy manipulation
    model = PPO.load(model_path, device="cpu")
    
    # Access the Policy's state dictionary (the weights)
    # We work directly on the PyTorch tensors
    policy = model.policy
    
    with torch.no_grad():
        # ---------------------------------------------------------
        # 1. Flip Convolutional Kernels (The Eyes)
        # ---------------------------------------------------------
        ...
        # ---------------------------------------------------------
        # 2. Reorder the First Linear Layer (The Optic Nerve)
        # ---------------------------------------------------------
        ...
        # Input: 84x84
        # Conv1 Output: 20x20 (64 channels) (Not exact, usually 32)
        # ...
        ...
        # usually named 'linear.0' or 'linear' inside features_extractor or mlp_extractor
        # In SB3 NatureCNN, the flattened output goes into 'linear[0]' of the features_extractor if it exists,
        # OR it feeds into the mlp_extractor. 
        
        # Let's find the first linear layer that connects to the CNN output.
        # In SB3 PPO CnnPolicy, the features_extractor outputs the features, then they go to mlp_extractor.
        # But NatureCNN in SB3 usually ends with a Linear(3136, 512).
        
        linear_layer = policy.features_extractor.linear[0]
        weights = linear_layer.weight.data # Shape: (512, 3136)
        
        # We assume the input format is (Channels, Height, Width) flattened.
        # We need to reshape, flip width, and flatten back.
        
        # Hardcoded for Standard Atari (84x84) NatureCNN
        n_channels = 64
        h = 7
        w = 7
